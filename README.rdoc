...
Since version 1.2.1, OWGM can send email alerts when an access point changes status from reachable to unreachable and vice versa.
...
Be sure to send emails from a server which is authorized to send email from the domain you use, otherwise the monitoring emails will be treated as SPAM.
For more information read here: http://en.wikipedia.org/wiki/MX_record
